...
BEGIN
    ...
    DBMS_OUTPUT.PUT_LINE('El nombre del cliente con c�digo ' || v_codigo || ' es ' || v_nombre);
EXCEPTION
    WHEN NO_DATA_FOUND THEN
        DBMS_OUTPUT.PUT_LINE('No se encontr� un cliente con el c�digo ' || v_codigo);
    WHEN OTHERS THEN
...
BEGIN
    ...
    SELECT precioVenta, gama
    INTO v_precioVenta, v_gama
    FROM productos
    WHERE codigo = v_codigo;

    
    DBMS_OUTPUT.PUT_LINE('El precio de venta del producto con código ' || v_codigo || ' es ' || v_precioVenta || ' y la gama es ' || v_gama);
EXCEPTION
    WHEN NO_DATA_FOUND THEN
        DBMS_OUTPUT.PUT_LINE('No se encontró un producto con el código ' || v_codigo);
    WHEN OTHERS THEN
...
DECLARE
    v_fechaEsperada pedidos.fechaEsperada%TYPE;
    v_fechaEntrega pedidos.fechaEntrega%TYPE;
    v_fechaPedido pedidos.fechaPedido%TYPE;
    v_estado pedidos.estado%TYPE;
    v_comentarios pedidos.comentarios%TYPE;
    v_codigo NUMBER := 1234;  
BEGIN
    
    SELECT fechaEsperada, fechaEntrega, fechaPedido, estado, comentarios
    INTO v_fechaEsperada, v_fechaEntrega, v_fechaPedido, v_estado, v_comentarios
    FROM pedidos
    WHERE codigo = v_codigo;

    
    DBMS_OUTPUT.PUT_LINE('Información del pedido con código ' || v_codigo || ':');
    DBMS_OUTPUT.PUT_LINE('Fecha Esperada: ' || v_fechaEsperada);
    DBMS_OUTPUT.PUT_LINE('Fecha de Entrega: ' || v_fechaEntrega);
    DBMS_OUTPUT.PUT_LINE('Fecha de Pedido: ' || v_fechaPedido);
    DBMS_OUTPUT.PUT_LINE('Estado: ' || v_estado);
    DBMS_OUTPUT.PUT_LINE('Comentarios: ' || v_comentarios);
EXCEPTION
    WHEN NO_DATA_FOUND THEN
        DBMS_OUTPUT.PUT_LINE('No se encontró un pedido con el código ' || v_codigo);
    WHEN OTHERS THEN
        DBMS_OUTPUT.PUT_LINE('Ocurrió un error: ' || SQLERRM);
END;
/
...
BEGIN
    ...
    DBMS_OUTPUT.PUT_LINE('El total del pedido con código ' || p_codigo_pedido || ' en euros es: ' || v_total);
EXCEPTION
    WHEN NO_DATA_FOUND THEN
        DBMS_OUTPUT.PUT_LINE('No se encontró el pedido con el código ' || p_codigo_pedido);
    WHEN OTHERS THEN
        DBMS_OUTPUT.PUT_LINE('Ocurrió un error: ' || SQLERRM);
END;
/
...
CREATE OR REPLACE PROCEDURE mostrar_nombre_cliente(p_codigo_cliente IN NUMBER) IS
...
BEGIN
    ...
    DBMS_OUTPUT.PUT_LINE('El nombre del cliente con código ' || p_codigo_cliente || ' es: ' || v_nombre_cliente);

EXCEPTION
    WHEN NO_DATA_FOUND THEN
      
        DBMS_OUTPUT.PUT_LINE('No se encontró un cliente con el código ' || p_codigo_cliente);
    WHEN OTHERS THEN
